...
# Theme: Improvements to the JuliaQuantum ecosystems

## [JuliaQuantum](http://juliaquantum.github.io) is a Julia umbrella organization to build libraries in Julia for Quantum science and technology. Through the fast development of last year--especially by successfully completing a GSoC project, the organization has implemented a framework of libraries to represent basic concepts in quantum mechanics and to solve foudamental quantum dynamics equations. However, there are still just a few breaks of chains to fully promote the existing projects to be as useful as other packages written in other programming languages. Implementing some binding packages with applications to existing packages are the focus of the ideal J/GSoC projects this year in JuliaQuantum:
...
- Optimizing the current quantum dynamics solvers and including more well-developed equation solvers for users to choose. Also building a plugin to let the solvers easily compatible with various parallel computing strategies. 
...
Discussions can be found in the [JuliaQuantum.github.io repo](https://github.com/JuliaQuantum/JuliaQuantum.github.io/issues/32). A current proposal can be found in [this PR](https://github.com/JuliaQuantum/Resources/pull/4). Mentors and helpers who are familiar with related areas and can do some code review works are highly welcome. 
